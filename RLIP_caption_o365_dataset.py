...
class Objects365Captions(torch.utils.data.Dataset):
    def __init__(self, img_set = 'val', img_folder = None, transforms = None):
        ...
        # bboxes = val_annos["annotations"][:5000] # for testing
        ...
        # For testing
        # id_to_filename = {str(img['id']):img['file_name'].split('/')[-2:] for img in (val_annos["images"])}
        valid_img_path_list = []
        valid_image_id_list = []
        ...
        self.valid_img_path_list = valid_img_path_list
        self.valid_image_id_list = valid_image_id_list
        self.ids = list(range(len(self.valid_image_id_list)))

    def __len__(self):
        return len(self.ids)
    
    def __getitem__(self, idx):
        img_path = self.valid_img_path_list[idx]
        img_id = self.valid_image_id_list[idx]

        raw_image = Image.open(img_path).convert('RGB')
        image = self._transforms(raw_image)

        return image, img_path, img_id
...
def generate_captions_for_images(
    ...
    if dataset == 'o365':
        ...
        o365_dataset = Objects365Captions(transforms = transform)

        sampler_val = torch.utils.data.RandomSampler(o365_dataset)
        batch_sampler_val = torch.utils.data.BatchSampler(
                    sampler_val, batch_size, drop_last=False)
        
        data_loader_val = DataLoader(o365_dataset,
    ...
    print_freq = 100
    for image, image_path_list, image_id_list in metric_logger.log_every(data_loader_val, print_freq, header):        
        image = image.tensors.to(device)
        with torch.no_grad():
            if not sampling:
